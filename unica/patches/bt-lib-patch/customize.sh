...
if xxd -p -c 0 "$WORK_DIR/system/system/lib64/libbluetooth_jni.so" | grep -q "39d9199428518152"; then
    ...
    LOG "\033[0;33m! No known VaultKeeper patch pattern matches the supplied libbluetooth_jni.so. Skipping VaultKeeper disable — both source and target are Samsung devices with native VaultKeeper support.\033[0m"
fi
